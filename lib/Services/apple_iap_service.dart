...
class AppleIAPService {
  ...
  Future<bool> initialize() async {
    ...
    // FIX 1: Initialize StoreKit properly
    if (Platform.isIOS) {
      // Set up the payment queue delegate
      final InAppPurchaseStoreKitPlatformAddition iosPlatformAddition =
          _inAppPurchase
              .getPlatformAddition<InAppPurchaseStoreKitPlatformAddition>();

      // This is crucial for simulator testing
    ...
    if (!isAvailable) {
      ...
      // In simulator, if StoreKit file is not configured, this returns false
      debugPrint(
    ...
    if (productsLoaded) {
      debugPrint(
          '✅ Apple IAP initialized successfully with ${_products.length} products');
    } else {
      debugPrint('⚠️ Apple IAP initialized but no products loaded');
      debugPrint(
          '💡 Check that product IDs in StoreKit Configuration file match your code');
    }
  ...
  Future<bool> loadProducts([List<String>? productIds]) async {
    ...
    try {
      ...
      debugPrint('📦 Loading products: $idsToLoad');
      ...
      if (response.error != null) {
        ...
        // FIX 2: More specific error handling for simulator
        if (response.error!.message.contains('SKErrorDomain') ||
            response.error!.message.contains('Cannot connect')) {
          debugPrint(
              '💡 This usually means StoreKit Configuration is not set up');
          debugPrint('💡 Steps to fix:');
          debugPrint('   1. Create a StoreKit Configuration file in Xcode');
          debugPrint('   2. Add your product IDs to the file');
          debugPrint('   3. Select the file in Edit Scheme > Run > Options');
        }
      ...
      if (response.notFoundIDs.isNotEmpty) {
        ...
        debugPrint(
            'ℹ️ Make sure these product IDs are in your StoreKit Configuration file:');
        for (var id in response.notFoundIDs) {
          debugPrint('   - $id');
        }
        Fluttertoast.showToast(
          msg: "Some subscriptions not found. Check StoreKit Configuration.",
          toastLength: Toast.LENGTH_LONG,
        );
      }
      ...
      if (_products.isEmpty) {
        ...
        debugPrint(
            '💡 Check that product IDs match between code and StoreKit Configuration');
        Fluttertoast.showToast(
          msg: "No subscriptions available. Check StoreKit Configuration.",
          toastLength: Toast.LENGTH_LONG,
        );
        return false;
      }
      ...
      Fluttertoast.showToast(
        msg: "Failed to load subscriptions: $e",
        toastLength: Toast.LENGTH_LONG,
      );
      return false;
    } finally {
  ...
  ProductDetails? findProductForSubscription(int subscriptionId) {
    ...
    try {
      ...
      debugPrint(
          '✅ Found product for subscription ID $subscriptionId: ${product.id}');
      return product;
    } catch (e) {
  ...
  Future<bool> buySubscriptionById(
    ...
    debugPrint('💳 buySubscriptionById - Subscription ID: $subscriptionId');
    ...
    debugPrint('✅ Product found: ${product.id}, proceeding with purchase');
  ...
  Future<bool> buySubscription(
    ...
    try {
      ...
      debugPrint("🔄 Calling Apple StoreKit...");

      // FIX 3: Use buyNonConsumable for auto-renewable subscriptions
      ...
      debugPrint('✅ Purchase request sent to StoreKit successfully');
      debugPrint('⏳ Waiting for StoreKit response...');

      // Set up watchdog timer (2 minutes for StoreKit)
  ...
  Future<bool> _verifyPurchase(PurchaseDetails purchaseDetails) async {
    ...
    debugPrint('📄 Receipt data length: ${receiptData.length} characters');
    ...
    try {
      ...
      debugPrint(
          'Backend verification: ${response.success ?? false ? "✅ SUCCESS" : "❌ FAILED"}');
...
// FIX 4: Add Payment Queue Delegate
